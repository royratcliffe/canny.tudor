...
## Database frames
And finally, what about data frame lazily-loaded from a database? Do types
manifest themselves? And do they appear _before_ or _after_ collection of data
by query execution? The simple answer is no, not without first collecting some
rows. This includes no rows, or limiting rows to zero.
...
```
Using R 4.1+ built-in pipe operator. Uses `dplyr` to read back the table lazily
and derive the types. No joy. You cannot access lazy table columns as you would
an in-memory data frame. Answers `NULL` because the frame is not yet available
locally.
```{r}
dplyr::tbl(db, "df")[["a"]]
